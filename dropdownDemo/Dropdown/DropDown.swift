...
//  DropDown
//
//  Created ByKevin Hirsch on 28/07/15.
...
public typealias SelectionClosure = (Index, DropDownDataModel) -> Void
public typealias ConfigurationClosure = (Index, DropDownDataModel) -> String
public typealias CellConfigurationClosure = (Index, DropDownDataModel, DropDownCell) -> Void
...
@objcMembers public final class CustomDropDown: UIView {
    ...
    public static weak var VisibleDropDown: CustomDropDown?
     ...
     */
    public var animationEntranceOptions: UIView.AnimationOptions = CustomDropDown.animationEntranceOptions
    
    /**
     The option of the hide animation. Only change the caller. To change all drop down's use the static var.
     */
    public var animationExitOptions: UIView.AnimationOptions = CustomDropDown.animationExitOptions
     ...
     */
    public var dataSource = [DropDownDataModel]() {
        didSet {
     ...
     */
    public var localizationKeysDataSource = [DropDownDataModel]() {
        didSet {
            //dataSource = localizationKeysDataSource.map { NSLocalizedString($0.item!, comment: "") }
        }
    }
     ...
     */
    public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [DropDownDataModel] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
        self.init(frame: .zero)
...
private extension CustomDropDown {
...
extension CustomDropDown {
    ...
    public override func updateConstraints() {
        ...
        //Bruce
        //we have increase 20 so that it is not out of iphone x top and we decrease 20 from height for the same reason.
    ...
    fileprivate func fittingWidth() -> CGFloat {
        ...
        for index in 0..<dataSource.count {
            ...
            //FIXME:- Fix IF any issue fasing change swift 4 to 4.2
            let width = templateCell.systemLayoutSizeFitting(.zero).width//.width//UILayoutFittingCompressedSize
...
extension CustomDropDown {
    ...
    public func show() -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
        if self == CustomDropDown.VisibleDropDown {
            return (true, 0)
        }
        
        if let visibleDropDown = CustomDropDown.VisibleDropDown {
            visibleDropDown.cancel()
        }
        
        willShowAction?()
        
        CustomDropDown.VisibleDropDown = self
    ...
    public func hide() {
        if self == CustomDropDown.VisibleDropDown {
             ...
             */
            CustomDropDown.VisibleDropDown = nil
        }
    ...
    @objc public func selectRowAtIndex(index: Int) {
...
extension CustomDropDown {
    ...
    public var selectedItem: DropDownDataModel? {
...
extension CustomDropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        if index >= 0 && index < localizationKeysDataSource.count {
            cell.accessibilityIdentifier = localizationKeysDataSource[index].item
        }
        ...
        let dropDownDM = dataSource[index] as DropDownDataModel
        if let cellConfiguration = cellConfiguration {
            cell.optionLabel.text = cellConfiguration(index, dropDownDM)
        } else {
            cell.optionLabel.text = dropDownDM.item
        }
        
        //        cell.optionLabel.textColor = dropDownDM.textColor
...
extension CustomDropDown {
    ...
    fileprivate func startListeningToKeyboard() {
       // KeyboardListener.sharedInstance.startListeningToKeyboard()
        ...
        //			self,
        //			selector: #selector(keyboardUpdate),
        //            name: UIResponder.NSNotification.Name.UIKeyboardDidHide,
        //			object: nil)
